...
func GetEnvVarConfig(key string) *config.SourcedConfig[string] {
	return config.NewFromEnvVar(key, os.Getenv(key))
}

func GetChalkClientArgConfig(value string) *config.SourcedConfig[string] {
	return config.NewFromArg(value)
}

func GetChalkYamlConfig(value string, configDir *string) *config.SourcedConfig[string] {
	var path string
	...
	return config.NewFromFile(path, value)
}
