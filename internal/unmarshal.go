...
func UnmarshalRecordInto(record arrow.Record, resultHolders any) (returnErr error) {
	defer func() {
	...
	numRows, err := Int64ToInt(record.NumRows())
	if err != nil {
		return errors.Newf("record too large to unmarshal, found %d rows", record.NumRows())
	}
	...
	for rowIdx := 0; rowIdx < recordRows; rowIdx++ {
		if err := rowOp(structs.Index(rowIdx), record, rowIdx); err != nil {
			return errors.Wrapf(err, "unmarshalling record")
		}
	}
