...
Set `TracerProvider` to trace `OnlineQueryBulk` request preparation, the RPC,
and result decoding with OpenTelemetry. Tracing is off by default.

```go
client, err := chalk.NewGRPCClient(ctx, &chalk.GRPCClientConfig{
    TracerProvider: otel.GetTracerProvider(),
    Tracing: &chalk.TracingOptions{
        Transport: true, // Optional transport diagnostics
    },
})
```
...
`TracingOptions.Transport` adds `chalk.online_query_bulk.http` spans with connection,
