// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        (unknown)
// source: chalk/server/v1/billing.proto

package serverv1

import (
	_ "github.com/chalk-ai/chalk-go/gen/chalk/auth/v1"
	v12 "github.com/chalk-ai/chalk-go/gen/chalk/kubernetes/v1"
	v11 "github.com/chalk-ai/chalk-go/gen/chalk/pubsub/v1"
	v1 "github.com/chalk-ai/chalk-go/gen/chalk/usage/v1"
	date "google.golang.org/genproto/googleapis/type/date"
	decimal "google.golang.org/genproto/googleapis/type/decimal"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type UsageChartPeriod int32

const (
	UsageChartPeriod_USAGE_CHART_PERIOD_UNSPECIFIED UsageChartPeriod = 0
	UsageChartPeriod_USAGE_CHART_PERIOD_DAILY       UsageChartPeriod = 1
	UsageChartPeriod_USAGE_CHART_PERIOD_MONTHLY     UsageChartPeriod = 2
	UsageChartPeriod_USAGE_CHART_PERIOD_HOURLY      UsageChartPeriod = 3
)

// Enum value maps for UsageChartPeriod.
var (
	UsageChartPeriod_name = map[int32]string{
		0: "USAGE_CHART_PERIOD_UNSPECIFIED",
		1: "USAGE_CHART_PERIOD_DAILY",
		2: "USAGE_CHART_PERIOD_MONTHLY",
		3: "USAGE_CHART_PERIOD_HOURLY",
	}
	UsageChartPeriod_value = map[string]int32{
		"USAGE_CHART_PERIOD_UNSPECIFIED": 0,
		"USAGE_CHART_PERIOD_DAILY":       1,
		"USAGE_CHART_PERIOD_MONTHLY":     2,
		"USAGE_CHART_PERIOD_HOURLY":      3,
	}
)

func (x UsageChartPeriod) Enum() *UsageChartPeriod {
	p := new(UsageChartPeriod)
	*p = x
	return p
}

func (x UsageChartPeriod) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (UsageChartPeriod) Descriptor() protoreflect.EnumDescriptor {
	return file_chalk_server_v1_billing_proto_enumTypes[0].Descriptor()
}

func (UsageChartPeriod) Type() protoreflect.EnumType {
	return &file_chalk_server_v1_billing_proto_enumTypes[0]
}

func (x UsageChartPeriod) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use UsageChartPeriod.Descriptor instead.
func (UsageChartPeriod) EnumDescriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{0}
}

type UsageChartGrouping int32

const (
	UsageChartGrouping_USAGE_CHART_GROUPING_UNSPECIFIED   UsageChartGrouping = 0
	UsageChartGrouping_USAGE_CHART_GROUPING_INSTANCE_TYPE UsageChartGrouping = 1
	UsageChartGrouping_USAGE_CHART_GROUPING_CLUSTER       UsageChartGrouping = 2
	UsageChartGrouping_USAGE_CHART_GROUPING_NODEPOOL      UsageChartGrouping = 3
	UsageChartGrouping_USAGE_CHART_GROUPING_WORKLOAD_TYPE UsageChartGrouping = 4
)

// Enum value maps for UsageChartGrouping.
var (
	UsageChartGrouping_name = map[int32]string{
		0: "USAGE_CHART_GROUPING_UNSPECIFIED",
		1: "USAGE_CHART_GROUPING_INSTANCE_TYPE",
		2: "USAGE_CHART_GROUPING_CLUSTER",
		3: "USAGE_CHART_GROUPING_NODEPOOL",
		4: "USAGE_CHART_GROUPING_WORKLOAD_TYPE",
	}
	UsageChartGrouping_value = map[string]int32{
		"USAGE_CHART_GROUPING_UNSPECIFIED":   0,
		"USAGE_CHART_GROUPING_INSTANCE_TYPE": 1,
		"USAGE_CHART_GROUPING_CLUSTER":       2,
		"USAGE_CHART_GROUPING_NODEPOOL":      3,
		"USAGE_CHART_GROUPING_WORKLOAD_TYPE": 4,
	}
)

func (x UsageChartGrouping) Enum() *UsageChartGrouping {
	p := new(UsageChartGrouping)
	*p = x
	return p
}

func (x UsageChartGrouping) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (UsageChartGrouping) Descriptor() protoreflect.EnumDescriptor {
	return file_chalk_server_v1_billing_proto_enumTypes[1].Descriptor()
}

func (UsageChartGrouping) Type() protoreflect.EnumType {
	return &file_chalk_server_v1_billing_proto_enumTypes[1]
}

func (x UsageChartGrouping) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use UsageChartGrouping.Descriptor instead.
func (UsageChartGrouping) EnumDescriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{1}
}

type UsageChartTimeRange int32

const (
	UsageChartTimeRange_USAGE_CHART_TIME_RANGE_UNSPECIFIED UsageChartTimeRange = 0
	UsageChartTimeRange_USAGE_CHART_TIME_RANGE_1D          UsageChartTimeRange = 1 // Last 1 day
	UsageChartTimeRange_USAGE_CHART_TIME_RANGE_7D          UsageChartTimeRange = 2 // Last 7 days
)

// Enum value maps for UsageChartTimeRange.
var (
	UsageChartTimeRange_name = map[int32]string{
		0: "USAGE_CHART_TIME_RANGE_UNSPECIFIED",
		1: "USAGE_CHART_TIME_RANGE_1D",
		2: "USAGE_CHART_TIME_RANGE_7D",
	}
	UsageChartTimeRange_value = map[string]int32{
		"USAGE_CHART_TIME_RANGE_UNSPECIFIED": 0,
		"USAGE_CHART_TIME_RANGE_1D":          1,
		"USAGE_CHART_TIME_RANGE_7D":          2,
	}
)

func (x UsageChartTimeRange) Enum() *UsageChartTimeRange {
	p := new(UsageChartTimeRange)
	*p = x
	return p
}

func (x UsageChartTimeRange) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (UsageChartTimeRange) Descriptor() protoreflect.EnumDescriptor {
	return file_chalk_server_v1_billing_proto_enumTypes[2].Descriptor()
}

func (UsageChartTimeRange) Type() protoreflect.EnumType {
	return &file_chalk_server_v1_billing_proto_enumTypes[2]
}

func (x UsageChartTimeRange) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use UsageChartTimeRange.Descriptor instead.
func (UsageChartTimeRange) EnumDescriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{2}
}

type GetUsageChartRequest struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	StartMs  *int64                 `protobuf:"varint,1,opt,name=start_ms,json=startMs,proto3,oneof" json:"start_ms,omitempty"`
	EndMs    *int64                 `protobuf:"varint,2,opt,name=end_ms,json=endMs,proto3,oneof" json:"end_ms,omitempty"`
	Period   *UsageChartPeriod      `protobuf:"varint,3,opt,name=period,proto3,enum=chalk.server.v1.UsageChartPeriod,oneof" json:"period,omitempty"`
	Grouping *UsageChartGrouping    `protobuf:"varint,4,opt,name=grouping,proto3,enum=chalk.server.v1.UsageChartGrouping,oneof" json:"grouping,omitempty"`
	// time_range is an alternative to start_ms/end_ms for common time ranges
	TimeRange     *UsageChartTimeRange `protobuf:"varint,5,opt,name=time_range,json=timeRange,proto3,enum=chalk.server.v1.UsageChartTimeRange,oneof" json:"time_range,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetUsageChartRequest) Reset() {
	*x = GetUsageChartRequest{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUsageChartRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUsageChartRequest) ProtoMessage() {}

func (x *GetUsageChartRequest) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUsageChartRequest.ProtoReflect.Descriptor instead.
func (*GetUsageChartRequest) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{0}
}

func (x *GetUsageChartRequest) GetStartMs() int64 {
	if x != nil && x.StartMs != nil {
		return *x.StartMs
	}
	return 0
}

func (x *GetUsageChartRequest) GetEndMs() int64 {
	if x != nil && x.EndMs != nil {
		return *x.EndMs
	}
	return 0
}

func (x *GetUsageChartRequest) GetPeriod() UsageChartPeriod {
	if x != nil && x.Period != nil {
		return *x.Period
	}
	return UsageChartPeriod_USAGE_CHART_PERIOD_UNSPECIFIED
}

func (x *GetUsageChartRequest) GetGrouping() UsageChartGrouping {
	if x != nil && x.Grouping != nil {
		return *x.Grouping
	}
	return UsageChartGrouping_USAGE_CHART_GROUPING_UNSPECIFIED
}

func (x *GetUsageChartRequest) GetTimeRange() UsageChartTimeRange {
	if x != nil && x.TimeRange != nil {
		return *x.TimeRange
	}
	return UsageChartTimeRange_USAGE_CHART_TIME_RANGE_UNSPECIFIED
}

type GetUsageChartResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Chart         *Chart                 `protobuf:"bytes,1,opt,name=chart,proto3" json:"chart,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetUsageChartResponse) Reset() {
	*x = GetUsageChartResponse{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUsageChartResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUsageChartResponse) ProtoMessage() {}

func (x *GetUsageChartResponse) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUsageChartResponse.ProtoReflect.Descriptor instead.
func (*GetUsageChartResponse) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{1}
}

func (x *GetUsageChartResponse) GetChart() *Chart {
	if x != nil {
		return x.Chart
	}
	return nil
}

type GetUtilizationRatesRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetUtilizationRatesRequest) Reset() {
	*x = GetUtilizationRatesRequest{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUtilizationRatesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUtilizationRatesRequest) ProtoMessage() {}

func (x *GetUtilizationRatesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUtilizationRatesRequest.ProtoReflect.Descriptor instead.
func (*GetUtilizationRatesRequest) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{2}
}

type GetUtilizationRatesResponse struct {
	state                         protoimpl.MessageState `protogen:"open.v1"`
	Rates                         []*v1.MachineRate      `protobuf:"bytes,1,rep,name=rates,proto3" json:"rates,omitempty"`
	SandboxCreditsPerVcpuHour     *decimal.Decimal       `protobuf:"bytes,2,opt,name=sandbox_credits_per_vcpu_hour,json=sandboxCreditsPerVcpuHour,proto3" json:"sandbox_credits_per_vcpu_hour,omitempty"`
	SandboxCreditsPerGbMemoryHour *decimal.Decimal       `protobuf:"bytes,3,opt,name=sandbox_credits_per_gb_memory_hour,json=sandboxCreditsPerGbMemoryHour,proto3" json:"sandbox_credits_per_gb_memory_hour,omitempty"`
	unknownFields                 protoimpl.UnknownFields
	sizeCache                     protoimpl.SizeCache
}

func (x *GetUtilizationRatesResponse) Reset() {
	*x = GetUtilizationRatesResponse{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUtilizationRatesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUtilizationRatesResponse) ProtoMessage() {}

func (x *GetUtilizationRatesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUtilizationRatesResponse.ProtoReflect.Descriptor instead.
func (*GetUtilizationRatesResponse) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{3}
}

func (x *GetUtilizationRatesResponse) GetRates() []*v1.MachineRate {
	if x != nil {
		return x.Rates
	}
	return nil
}

func (x *GetUtilizationRatesResponse) GetSandboxCreditsPerVcpuHour() *decimal.Decimal {
	if x != nil {
		return x.SandboxCreditsPerVcpuHour
	}
	return nil
}

func (x *GetUtilizationRatesResponse) GetSandboxCreditsPerGbMemoryHour() *decimal.Decimal {
	if x != nil {
		return x.SandboxCreditsPerGbMemoryHour
	}
	return nil
}

type GetAvailableInstanceTypesRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetAvailableInstanceTypesRequest) Reset() {
	*x = GetAvailableInstanceTypesRequest{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAvailableInstanceTypesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAvailableInstanceTypesRequest) ProtoMessage() {}

func (x *GetAvailableInstanceTypesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAvailableInstanceTypesRequest.ProtoReflect.Descriptor instead.
func (*GetAvailableInstanceTypesRequest) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{4}
}

type GetAvailableInstanceTypesResponse struct {
	state         protoimpl.MessageState  `protogen:"open.v1"`
	InstanceTypes []*v1.CloudInstanceType `protobuf:"bytes,1,rep,name=instance_types,json=instanceTypes,proto3" json:"instance_types,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetAvailableInstanceTypesResponse) Reset() {
	*x = GetAvailableInstanceTypesResponse{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAvailableInstanceTypesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAvailableInstanceTypesResponse) ProtoMessage() {}

func (x *GetAvailableInstanceTypesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAvailableInstanceTypesResponse.ProtoReflect.Descriptor instead.
func (*GetAvailableInstanceTypesResponse) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{5}
}

func (x *GetAvailableInstanceTypesResponse) GetInstanceTypes() []*v1.CloudInstanceType {
	if x != nil {
		return x.InstanceTypes
	}
	return nil
}

type GetNodesAndPodsRequest struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	Namespace        *string                `protobuf:"bytes,1,opt,name=namespace,proto3,oneof" json:"namespace,omitempty"`
	PodLabelSelector *string                `protobuf:"bytes,2,opt,name=pod_label_selector,json=podLabelSelector,proto3,oneof" json:"pod_label_selector,omitempty"`
	EnvironmentId    *string                `protobuf:"bytes,3,opt,name=environment_id,json=environmentId,proto3,oneof" json:"environment_id,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *GetNodesAndPodsRequest) Reset() {
	*x = GetNodesAndPodsRequest{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetNodesAndPodsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetNodesAndPodsRequest) ProtoMessage() {}

func (x *GetNodesAndPodsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetNodesAndPodsRequest.ProtoReflect.Descriptor instead.
func (*GetNodesAndPodsRequest) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{6}
}

func (x *GetNodesAndPodsRequest) GetNamespace() string {
	if x != nil && x.Namespace != nil {
		return *x.Namespace
	}
	return ""
}

func (x *GetNodesAndPodsRequest) GetPodLabelSelector() string {
	if x != nil && x.PodLabelSelector != nil {
		return *x.PodLabelSelector
	}
	return ""
}

func (x *GetNodesAndPodsRequest) GetEnvironmentId() string {
	if x != nil && x.EnvironmentId != nil {
		return *x.EnvironmentId
	}
	return ""
}

type GetNodesAndPodsResponse struct {
	state         protoimpl.MessageState  `protogen:"open.v1"`
	Nodes         []*v11.NodeStatusPubSub `protobuf:"bytes,1,rep,name=nodes,proto3" json:"nodes,omitempty"`
	Pods          []*v11.PodStatusPubSub  `protobuf:"bytes,2,rep,name=pods,proto3" json:"pods,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetNodesAndPodsResponse) Reset() {
	*x = GetNodesAndPodsResponse{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetNodesAndPodsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetNodesAndPodsResponse) ProtoMessage() {}

func (x *GetNodesAndPodsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetNodesAndPodsResponse.ProtoReflect.Descriptor instead.
func (*GetNodesAndPodsResponse) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{7}
}

func (x *GetNodesAndPodsResponse) GetNodes() []*v11.NodeStatusPubSub {
	if x != nil {
		return x.Nodes
	}
	return nil
}

func (x *GetNodesAndPodsResponse) GetPods() []*v11.PodStatusPubSub {
	if x != nil {
		return x.Pods
	}
	return nil
}

type PublishNodeUsageRequest struct {
	state         protoimpl.MessageState  `protogen:"open.v1"`
	Nodes         []*v11.NodeStatusPubSub `protobuf:"bytes,1,rep,name=nodes,proto3" json:"nodes,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PublishNodeUsageRequest) Reset() {
	*x = PublishNodeUsageRequest{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PublishNodeUsageRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PublishNodeUsageRequest) ProtoMessage() {}

func (x *PublishNodeUsageRequest) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PublishNodeUsageRequest.ProtoReflect.Descriptor instead.
func (*PublishNodeUsageRequest) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{8}
}

func (x *PublishNodeUsageRequest) GetNodes() []*v11.NodeStatusPubSub {
	if x != nil {
		return x.Nodes
	}
	return nil
}

type PublishNodeUsageResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PublishNodeUsageResponse) Reset() {
	*x = PublishNodeUsageResponse{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PublishNodeUsageResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PublishNodeUsageResponse) ProtoMessage() {}

func (x *PublishNodeUsageResponse) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PublishNodeUsageResponse.ProtoReflect.Descriptor instead.
func (*PublishNodeUsageResponse) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{9}
}

type GetNodesAndPodsUIRequest struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	Namespace        *string                `protobuf:"bytes,1,opt,name=namespace,proto3,oneof" json:"namespace,omitempty"`
	PodLabelSelector *string                `protobuf:"bytes,2,opt,name=pod_label_selector,json=podLabelSelector,proto3,oneof" json:"pod_label_selector,omitempty"`
	EnvironmentId    *string                `protobuf:"bytes,3,opt,name=environment_id,json=environmentId,proto3,oneof" json:"environment_id,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *GetNodesAndPodsUIRequest) Reset() {
	*x = GetNodesAndPodsUIRequest{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetNodesAndPodsUIRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetNodesAndPodsUIRequest) ProtoMessage() {}

func (x *GetNodesAndPodsUIRequest) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetNodesAndPodsUIRequest.ProtoReflect.Descriptor instead.
func (*GetNodesAndPodsUIRequest) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{10}
}

func (x *GetNodesAndPodsUIRequest) GetNamespace() string {
	if x != nil && x.Namespace != nil {
		return *x.Namespace
	}
	return ""
}

func (x *GetNodesAndPodsUIRequest) GetPodLabelSelector() string {
	if x != nil && x.PodLabelSelector != nil {
		return *x.PodLabelSelector
	}
	return ""
}

func (x *GetNodesAndPodsUIRequest) GetEnvironmentId() string {
	if x != nil && x.EnvironmentId != nil {
		return *x.EnvironmentId
	}
	return ""
}

type GetNodesAndPodsUIResponse struct {
	state         protoimpl.MessageState    `protogen:"open.v1"`
	Nodes         []*v12.KubernetesNodeData `protobuf:"bytes,1,rep,name=nodes,proto3" json:"nodes,omitempty"`
	Pods          []*v12.KubernetesPodData  `protobuf:"bytes,2,rep,name=pods,proto3" json:"pods,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetNodesAndPodsUIResponse) Reset() {
	*x = GetNodesAndPodsUIResponse{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetNodesAndPodsUIResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetNodesAndPodsUIResponse) ProtoMessage() {}

func (x *GetNodesAndPodsUIResponse) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetNodesAndPodsUIResponse.ProtoReflect.Descriptor instead.
func (*GetNodesAndPodsUIResponse) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{11}
}

func (x *GetNodesAndPodsUIResponse) GetNodes() []*v12.KubernetesNodeData {
	if x != nil {
		return x.Nodes
	}
	return nil
}

func (x *GetNodesAndPodsUIResponse) GetPods() []*v12.KubernetesPodData {
	if x != nil {
		return x.Pods
	}
	return nil
}

type SyncUtilizationRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SyncUtilizationRequest) Reset() {
	*x = SyncUtilizationRequest{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SyncUtilizationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SyncUtilizationRequest) ProtoMessage() {}

func (x *SyncUtilizationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SyncUtilizationRequest.ProtoReflect.Descriptor instead.
func (*SyncUtilizationRequest) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{12}
}

type SyncUtilizationResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SyncUtilizationResponse) Reset() {
	*x = SyncUtilizationResponse{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SyncUtilizationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SyncUtilizationResponse) ProtoMessage() {}

func (x *SyncUtilizationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SyncUtilizationResponse.ProtoReflect.Descriptor instead.
func (*SyncUtilizationResponse) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{13}
}

type GetCreditBundlesRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetCreditBundlesRequest) Reset() {
	*x = GetCreditBundlesRequest{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetCreditBundlesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetCreditBundlesRequest) ProtoMessage() {}

func (x *GetCreditBundlesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetCreditBundlesRequest.ProtoReflect.Descriptor instead.
func (*GetCreditBundlesRequest) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{14}
}

type GetCreditBundlesResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Bundles       []*CreditBundle        `protobuf:"bytes,1,rep,name=bundles,proto3" json:"bundles,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetCreditBundlesResponse) Reset() {
	*x = GetCreditBundlesResponse{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetCreditBundlesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetCreditBundlesResponse) ProtoMessage() {}

func (x *GetCreditBundlesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetCreditBundlesResponse.ProtoReflect.Descriptor instead.
func (*GetCreditBundlesResponse) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{15}
}

func (x *GetCreditBundlesResponse) GetBundles() []*CreditBundle {
	if x != nil {
		return x.Bundles
	}
	return nil
}

type CreditBundle struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	BundleId         string                 `protobuf:"bytes,1,opt,name=bundle_id,json=bundleId,proto3" json:"bundle_id,omitempty"`
	PurchaseDate     *date.Date             `protobuf:"bytes,2,opt,name=purchase_date,json=purchaseDate,proto3" json:"purchase_date,omitempty"`
	CreditQuantity   int32                  `protobuf:"varint,3,opt,name=credit_quantity,json=creditQuantity,proto3" json:"credit_quantity,omitempty"`
	PurchasePrice    int32                  `protobuf:"varint,4,opt,name=purchase_price,json=purchasePrice,proto3" json:"purchase_price,omitempty"`
	ExpiresOn        *date.Date             `protobuf:"bytes,5,opt,name=expires_on,json=expiresOn,proto3,oneof" json:"expires_on,omitempty"`
	RemainingCredits int32                  `protobuf:"varint,6,opt,name=remaining_credits,json=remainingCredits,proto3" json:"remaining_credits,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *CreditBundle) Reset() {
	*x = CreditBundle{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreditBundle) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreditBundle) ProtoMessage() {}

func (x *CreditBundle) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreditBundle.ProtoReflect.Descriptor instead.
func (*CreditBundle) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{16}
}

func (x *CreditBundle) GetBundleId() string {
	if x != nil {
		return x.BundleId
	}
	return ""
}

func (x *CreditBundle) GetPurchaseDate() *date.Date {
	if x != nil {
		return x.PurchaseDate
	}
	return nil
}

func (x *CreditBundle) GetCreditQuantity() int32 {
	if x != nil {
		return x.CreditQuantity
	}
	return 0
}

func (x *CreditBundle) GetPurchasePrice() int32 {
	if x != nil {
		return x.PurchasePrice
	}
	return 0
}

func (x *CreditBundle) GetExpiresOn() *date.Date {
	if x != nil {
		return x.ExpiresOn
	}
	return nil
}

func (x *CreditBundle) GetRemainingCredits() int32 {
	if x != nil {
		return x.RemainingCredits
	}
	return 0
}

type GetInstanceUsageRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StartMs       *int64                 `protobuf:"varint,1,opt,name=start_ms,json=startMs,proto3,oneof" json:"start_ms,omitempty"`
	EndMs         *int64                 `protobuf:"varint,2,opt,name=end_ms,json=endMs,proto3,oneof" json:"end_ms,omitempty"`
	EnvironmentId *string                `protobuf:"bytes,3,opt,name=environment_id,json=environmentId,proto3,oneof" json:"environment_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetInstanceUsageRequest) Reset() {
	*x = GetInstanceUsageRequest{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetInstanceUsageRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetInstanceUsageRequest) ProtoMessage() {}

func (x *GetInstanceUsageRequest) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetInstanceUsageRequest.ProtoReflect.Descriptor instead.
func (*GetInstanceUsageRequest) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{17}
}

func (x *GetInstanceUsageRequest) GetStartMs() int64 {
	if x != nil && x.StartMs != nil {
		return *x.StartMs
	}
	return 0
}

func (x *GetInstanceUsageRequest) GetEndMs() int64 {
	if x != nil && x.EndMs != nil {
		return *x.EndMs
	}
	return 0
}

func (x *GetInstanceUsageRequest) GetEnvironmentId() string {
	if x != nil && x.EnvironmentId != nil {
		return *x.EnvironmentId
	}
	return ""
}

type InstanceUsage struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ClusterName   string                 `protobuf:"bytes,1,opt,name=cluster_name,json=clusterName,proto3" json:"cluster_name,omitempty"`
	InstanceType  string                 `protobuf:"bytes,2,opt,name=instance_type,json=instanceType,proto3" json:"instance_type,omitempty"`
	Id            string                 `protobuf:"bytes,3,opt,name=id,proto3" json:"id,omitempty"`
	StartTime     *timestamppb.Timestamp `protobuf:"bytes,4,opt,name=start_time,json=startTime,proto3" json:"start_time,omitempty"`
	EndTime       *timestamppb.Timestamp `protobuf:"bytes,5,opt,name=end_time,json=endTime,proto3" json:"end_time,omitempty"`
	HoursUsed     float64                `protobuf:"fixed64,6,opt,name=hours_used,json=hoursUsed,proto3" json:"hours_used,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *InstanceUsage) Reset() {
	*x = InstanceUsage{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *InstanceUsage) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*InstanceUsage) ProtoMessage() {}

func (x *InstanceUsage) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use InstanceUsage.ProtoReflect.Descriptor instead.
func (*InstanceUsage) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{18}
}

func (x *InstanceUsage) GetClusterName() string {
	if x != nil {
		return x.ClusterName
	}
	return ""
}

func (x *InstanceUsage) GetInstanceType() string {
	if x != nil {
		return x.InstanceType
	}
	return ""
}

func (x *InstanceUsage) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *InstanceUsage) GetStartTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StartTime
	}
	return nil
}

func (x *InstanceUsage) GetEndTime() *timestamppb.Timestamp {
	if x != nil {
		return x.EndTime
	}
	return nil
}

func (x *InstanceUsage) GetHoursUsed() float64 {
	if x != nil {
		return x.HoursUsed
	}
	return 0
}

type GetInstanceUsageResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Instances     []*InstanceUsage       `protobuf:"bytes,1,rep,name=instances,proto3" json:"instances,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetInstanceUsageResponse) Reset() {
	*x = GetInstanceUsageResponse{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetInstanceUsageResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetInstanceUsageResponse) ProtoMessage() {}

func (x *GetInstanceUsageResponse) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetInstanceUsageResponse.ProtoReflect.Descriptor instead.
func (*GetInstanceUsageResponse) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{19}
}

func (x *GetInstanceUsageResponse) GetInstances() []*InstanceUsage {
	if x != nil {
		return x.Instances
	}
	return nil
}

// Request message for GetPodTimeRanges
type GetPodTimeRangesRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// List of pod names to query time ranges for (exact match)
	PodNames []string `protobuf:"bytes,1,rep,name=pod_names,json=podNames,proto3" json:"pod_names,omitempty"`
	// Regex pattern to match pod names (alternative to pod_names)
	PodNameRegex *string `protobuf:"bytes,2,opt,name=pod_name_regex,json=podNameRegex,proto3,oneof" json:"pod_name_regex,omitempty"`
	// Filter by resource group label (chalk.ai/resource-group)
	ResourceGroup *string `protobuf:"bytes,3,opt,name=resource_group,json=resourceGroup,proto3,oneof" json:"resource_group,omitempty"`
	// Filter by nodepool label
	Nodepool *string `protobuf:"bytes,4,opt,name=nodepool,proto3,oneof" json:"nodepool,omitempty"`
	// Filter by component label
	Component *string `protobuf:"bytes,5,opt,name=component,proto3,oneof" json:"component,omitempty"`
	// Filter by service kind label (chalk.ai/service-kind)
	ServiceKind *string `protobuf:"bytes,6,opt,name=service_kind,json=serviceKind,proto3,oneof" json:"service_kind,omitempty"`
	// Start time filter - only include pods observed after this time
	StartTime *timestamppb.Timestamp `protobuf:"bytes,7,opt,name=start_time,json=startTime,proto3,oneof" json:"start_time,omitempty"`
	// End time filter - only include pods observed before this time
	EndTime       *timestamppb.Timestamp `protobuf:"bytes,8,opt,name=end_time,json=endTime,proto3,oneof" json:"end_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetPodTimeRangesRequest) Reset() {
	*x = GetPodTimeRangesRequest{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetPodTimeRangesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetPodTimeRangesRequest) ProtoMessage() {}

func (x *GetPodTimeRangesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetPodTimeRangesRequest.ProtoReflect.Descriptor instead.
func (*GetPodTimeRangesRequest) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{20}
}

func (x *GetPodTimeRangesRequest) GetPodNames() []string {
	if x != nil {
		return x.PodNames
	}
	return nil
}

func (x *GetPodTimeRangesRequest) GetPodNameRegex() string {
	if x != nil && x.PodNameRegex != nil {
		return *x.PodNameRegex
	}
	return ""
}

func (x *GetPodTimeRangesRequest) GetResourceGroup() string {
	if x != nil && x.ResourceGroup != nil {
		return *x.ResourceGroup
	}
	return ""
}

func (x *GetPodTimeRangesRequest) GetNodepool() string {
	if x != nil && x.Nodepool != nil {
		return *x.Nodepool
	}
	return ""
}

func (x *GetPodTimeRangesRequest) GetComponent() string {
	if x != nil && x.Component != nil {
		return *x.Component
	}
	return ""
}

func (x *GetPodTimeRangesRequest) GetServiceKind() string {
	if x != nil && x.ServiceKind != nil {
		return *x.ServiceKind
	}
	return ""
}

func (x *GetPodTimeRangesRequest) GetStartTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StartTime
	}
	return nil
}

func (x *GetPodTimeRangesRequest) GetEndTime() *timestamppb.Timestamp {
	if x != nil {
		return x.EndTime
	}
	return nil
}

// Time range for a single pod
type PodTimeRange struct {
	state     protoimpl.MessageState `protogen:"open.v1"`
	PodName   string                 `protobuf:"bytes,1,opt,name=pod_name,json=podName,proto3" json:"pod_name,omitempty"`
	StartTime *timestamppb.Timestamp `protobuf:"bytes,2,opt,name=start_time,json=startTime,proto3" json:"start_time,omitempty"`
	EndTime   *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=end_time,json=endTime,proto3" json:"end_time,omitempty"`
	// The node this pod was scheduled on, if known. Sourced from spec.node_name.
	NodeName string `protobuf:"bytes,4,opt,name=node_name,json=nodeName,proto3" json:"node_name,omitempty"`
	// The instance type of the node this pod was scheduled on, if known.
	// Joined from the nodes table on (team, cluster, node_name).
	InstanceType string `protobuf:"bytes,5,opt,name=instance_type,json=instanceType,proto3" json:"instance_type,omitempty"`
	// Sum of the memory resource requests (bytes) across the pod's containers,
	// as observed while it was running. Summed from spec.containers[].resources.requests.
	MemoryRequestBytes int64 `protobuf:"varint,6,opt,name=memory_request_bytes,json=memoryRequestBytes,proto3" json:"memory_request_bytes,omitempty"`
	// Sum of the CPU resource requests (millicores) across the pod's containers,
	// as observed while it was running. Summed from spec.containers[].resources.requests.
	CpuRequestMillicores int64 `protobuf:"varint,7,opt,name=cpu_request_millicores,json=cpuRequestMillicores,proto3" json:"cpu_request_millicores,omitempty"`
	// The pod's labels, as observed while it was running.
	Labels        map[string]string `protobuf:"bytes,8,rep,name=labels,proto3" json:"labels,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PodTimeRange) Reset() {
	*x = PodTimeRange{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PodTimeRange) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PodTimeRange) ProtoMessage() {}

func (x *PodTimeRange) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PodTimeRange.ProtoReflect.Descriptor instead.
func (*PodTimeRange) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{21}
}

func (x *PodTimeRange) GetPodName() string {
	if x != nil {
		return x.PodName
	}
	return ""
}

func (x *PodTimeRange) GetStartTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StartTime
	}
	return nil
}

func (x *PodTimeRange) GetEndTime() *timestamppb.Timestamp {
	if x != nil {
		return x.EndTime
	}
	return nil
}

func (x *PodTimeRange) GetNodeName() string {
	if x != nil {
		return x.NodeName
	}
	return ""
}

func (x *PodTimeRange) GetInstanceType() string {
	if x != nil {
		return x.InstanceType
	}
	return ""
}

func (x *PodTimeRange) GetMemoryRequestBytes() int64 {
	if x != nil {
		return x.MemoryRequestBytes
	}
	return 0
}

func (x *PodTimeRange) GetCpuRequestMillicores() int64 {
	if x != nil {
		return x.CpuRequestMillicores
	}
	return 0
}

func (x *PodTimeRange) GetLabels() map[string]string {
	if x != nil {
		return x.Labels
	}
	return nil
}

// Response message for GetPodTimeRanges
type GetPodTimeRangesResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TimeRanges    []*PodTimeRange        `protobuf:"bytes,1,rep,name=time_ranges,json=timeRanges,proto3" json:"time_ranges,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetPodTimeRangesResponse) Reset() {
	*x = GetPodTimeRangesResponse{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[22]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetPodTimeRangesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetPodTimeRangesResponse) ProtoMessage() {}

func (x *GetPodTimeRangesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetPodTimeRangesResponse.ProtoReflect.Descriptor instead.
func (*GetPodTimeRangesResponse) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{22}
}

func (x *GetPodTimeRangesResponse) GetTimeRanges() []*PodTimeRange {
	if x != nil {
		return x.TimeRanges
	}
	return nil
}

// Request message for GetNodeTimeRanges
type GetNodeTimeRangesRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// List of node names to query time ranges for (exact match)
	NodeNames []string `protobuf:"bytes,1,rep,name=node_names,json=nodeNames,proto3" json:"node_names,omitempty"`
	// Regex pattern to match node names (alternative to node_names)
	NodeNameRegex *string `protobuf:"bytes,2,opt,name=node_name_regex,json=nodeNameRegex,proto3,oneof" json:"node_name_regex,omitempty"`
	// Filter by nodepool label
	Nodepool *string `protobuf:"bytes,3,opt,name=nodepool,proto3,oneof" json:"nodepool,omitempty"`
	// Start time filter - only include nodes observed after this time
	StartTime *timestamppb.Timestamp `protobuf:"bytes,4,opt,name=start_time,json=startTime,proto3,oneof" json:"start_time,omitempty"`
	// End time filter - only include nodes observed before this time
	EndTime       *timestamppb.Timestamp `protobuf:"bytes,5,opt,name=end_time,json=endTime,proto3,oneof" json:"end_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetNodeTimeRangesRequest) Reset() {
	*x = GetNodeTimeRangesRequest{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetNodeTimeRangesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetNodeTimeRangesRequest) ProtoMessage() {}

func (x *GetNodeTimeRangesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetNodeTimeRangesRequest.ProtoReflect.Descriptor instead.
func (*GetNodeTimeRangesRequest) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{23}
}

func (x *GetNodeTimeRangesRequest) GetNodeNames() []string {
	if x != nil {
		return x.NodeNames
	}
	return nil
}

func (x *GetNodeTimeRangesRequest) GetNodeNameRegex() string {
	if x != nil && x.NodeNameRegex != nil {
		return *x.NodeNameRegex
	}
	return ""
}

func (x *GetNodeTimeRangesRequest) GetNodepool() string {
	if x != nil && x.Nodepool != nil {
		return *x.Nodepool
	}
	return ""
}

func (x *GetNodeTimeRangesRequest) GetStartTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StartTime
	}
	return nil
}

func (x *GetNodeTimeRangesRequest) GetEndTime() *timestamppb.Timestamp {
	if x != nil {
		return x.EndTime
	}
	return nil
}

// Time range for a single node
type NodeTimeRange struct {
	state     protoimpl.MessageState `protogen:"open.v1"`
	NodeName  string                 `protobuf:"bytes,1,opt,name=node_name,json=nodeName,proto3" json:"node_name,omitempty"`
	StartTime *timestamppb.Timestamp `protobuf:"bytes,2,opt,name=start_time,json=startTime,proto3" json:"start_time,omitempty"`
	EndTime   *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=end_time,json=endTime,proto3" json:"end_time,omitempty"`
	// The Kubernetes UID of the node.
	NodeUid       string `protobuf:"bytes,4,opt,name=node_uid,json=nodeUid,proto3" json:"node_uid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *NodeTimeRange) Reset() {
	*x = NodeTimeRange{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[24]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *NodeTimeRange) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*NodeTimeRange) ProtoMessage() {}

func (x *NodeTimeRange) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[24]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use NodeTimeRange.ProtoReflect.Descriptor instead.
func (*NodeTimeRange) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{24}
}

func (x *NodeTimeRange) GetNodeName() string {
	if x != nil {
		return x.NodeName
	}
	return ""
}

func (x *NodeTimeRange) GetStartTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StartTime
	}
	return nil
}

func (x *NodeTimeRange) GetEndTime() *timestamppb.Timestamp {
	if x != nil {
		return x.EndTime
	}
	return nil
}

func (x *NodeTimeRange) GetNodeUid() string {
	if x != nil {
		return x.NodeUid
	}
	return ""
}

// Response message for GetNodeTimeRanges
type GetNodeTimeRangesResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TimeRanges    []*NodeTimeRange       `protobuf:"bytes,1,rep,name=time_ranges,json=timeRanges,proto3" json:"time_ranges,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetNodeTimeRangesResponse) Reset() {
	*x = GetNodeTimeRangesResponse{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[25]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetNodeTimeRangesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetNodeTimeRangesResponse) ProtoMessage() {}

func (x *GetNodeTimeRangesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[25]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetNodeTimeRangesResponse.ProtoReflect.Descriptor instead.
func (*GetNodeTimeRangesResponse) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{25}
}

func (x *GetNodeTimeRangesResponse) GetTimeRanges() []*NodeTimeRange {
	if x != nil {
		return x.TimeRanges
	}
	return nil
}

// Request message for GetNodeDetail
type GetNodeDetailRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The name of the kube cluster the node belongs to
	ClusterName string `protobuf:"bytes,1,opt,name=cluster_name,json=clusterName,proto3" json:"cluster_name,omitempty"`
	// The name of the node to look up
	NodeName string `protobuf:"bytes,2,opt,name=node_name,json=nodeName,proto3" json:"node_name,omitempty"`
	// The Kubernetes UID of the node. When provided, the lookup uses uid instead
	// of node_name, which avoids ambiguity when node names are reused (e.g. EKS
	// nodes named by IP address).
	NodeUid       *string `protobuf:"bytes,3,opt,name=node_uid,json=nodeUid,proto3,oneof" json:"node_uid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetNodeDetailRequest) Reset() {
	*x = GetNodeDetailRequest{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[26]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetNodeDetailRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetNodeDetailRequest) ProtoMessage() {}

func (x *GetNodeDetailRequest) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[26]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetNodeDetailRequest.ProtoReflect.Descriptor instead.
func (*GetNodeDetailRequest) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{26}
}

func (x *GetNodeDetailRequest) GetClusterName() string {
	if x != nil {
		return x.ClusterName
	}
	return ""
}

func (x *GetNodeDetailRequest) GetNodeName() string {
	if x != nil {
		return x.NodeName
	}
	return ""
}

func (x *GetNodeDetailRequest) GetNodeUid() string {
	if x != nil && x.NodeUid != nil {
		return *x.NodeUid
	}
	return ""
}

// Information about a node from usage data
type NodeDetailInfo struct {
	state        protoimpl.MessageState `protogen:"open.v1"`
	NodeName     string                 `protobuf:"bytes,1,opt,name=node_name,json=nodeName,proto3" json:"node_name,omitempty"`
	ClusterName  string                 `protobuf:"bytes,2,opt,name=cluster_name,json=clusterName,proto3" json:"cluster_name,omitempty"`
	Nodepool     string                 `protobuf:"bytes,3,opt,name=nodepool,proto3" json:"nodepool,omitempty"`
	InstanceType string                 `protobuf:"bytes,4,opt,name=instance_type,json=instanceType,proto3" json:"instance_type,omitempty"`
	StartTime    *timestamppb.Timestamp `protobuf:"bytes,5,opt,name=start_time,json=startTime,proto3" json:"start_time,omitempty"`
	EndTime      *timestamppb.Timestamp `protobuf:"bytes,6,opt,name=end_time,json=endTime,proto3" json:"end_time,omitempty"`
	// The Kubernetes UID of the node.
	NodeUid       string `protobuf:"bytes,7,opt,name=node_uid,json=nodeUid,proto3" json:"node_uid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *NodeDetailInfo) Reset() {
	*x = NodeDetailInfo{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[27]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *NodeDetailInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*NodeDetailInfo) ProtoMessage() {}

func (x *NodeDetailInfo) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[27]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use NodeDetailInfo.ProtoReflect.Descriptor instead.
func (*NodeDetailInfo) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{27}
}

func (x *NodeDetailInfo) GetNodeName() string {
	if x != nil {
		return x.NodeName
	}
	return ""
}

func (x *NodeDetailInfo) GetClusterName() string {
	if x != nil {
		return x.ClusterName
	}
	return ""
}

func (x *NodeDetailInfo) GetNodepool() string {
	if x != nil {
		return x.Nodepool
	}
	return ""
}

func (x *NodeDetailInfo) GetInstanceType() string {
	if x != nil {
		return x.InstanceType
	}
	return ""
}

func (x *NodeDetailInfo) GetStartTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StartTime
	}
	return nil
}

func (x *NodeDetailInfo) GetEndTime() *timestamppb.Timestamp {
	if x != nil {
		return x.EndTime
	}
	return nil
}

func (x *NodeDetailInfo) GetNodeUid() string {
	if x != nil {
		return x.NodeUid
	}
	return ""
}

// Information about a pod that was scheduled on a node
type NodeDetailPod struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	PodName       string                 `protobuf:"bytes,1,opt,name=pod_name,json=podName,proto3" json:"pod_name,omitempty"`
	StartTime     *timestamppb.Timestamp `protobuf:"bytes,2,opt,name=start_time,json=startTime,proto3" json:"start_time,omitempty"`
	EndTime       *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=end_time,json=endTime,proto3" json:"end_time,omitempty"`
	WorkloadType  string                 `protobuf:"bytes,4,opt,name=workload_type,json=workloadType,proto3" json:"workload_type,omitempty"`
	ResourceGroup string                 `protobuf:"bytes,5,opt,name=resource_group,json=resourceGroup,proto3" json:"resource_group,omitempty"`
	CpuRequest    string                 `protobuf:"bytes,6,opt,name=cpu_request,json=cpuRequest,proto3" json:"cpu_request,omitempty"`
	CpuLimit      string                 `protobuf:"bytes,7,opt,name=cpu_limit,json=cpuLimit,proto3" json:"cpu_limit,omitempty"`
	MemoryRequest string                 `protobuf:"bytes,8,opt,name=memory_request,json=memoryRequest,proto3" json:"memory_request,omitempty"`
	MemoryLimit   string                 `protobuf:"bytes,9,opt,name=memory_limit,json=memoryLimit,proto3" json:"memory_limit,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *NodeDetailPod) Reset() {
	*x = NodeDetailPod{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[28]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *NodeDetailPod) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*NodeDetailPod) ProtoMessage() {}

func (x *NodeDetailPod) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[28]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use NodeDetailPod.ProtoReflect.Descriptor instead.
func (*NodeDetailPod) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{28}
}

func (x *NodeDetailPod) GetPodName() string {
	if x != nil {
		return x.PodName
	}
	return ""
}

func (x *NodeDetailPod) GetStartTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StartTime
	}
	return nil
}

func (x *NodeDetailPod) GetEndTime() *timestamppb.Timestamp {
	if x != nil {
		return x.EndTime
	}
	return nil
}

func (x *NodeDetailPod) GetWorkloadType() string {
	if x != nil {
		return x.WorkloadType
	}
	return ""
}

func (x *NodeDetailPod) GetResourceGroup() string {
	if x != nil {
		return x.ResourceGroup
	}
	return ""
}

func (x *NodeDetailPod) GetCpuRequest() string {
	if x != nil {
		return x.CpuRequest
	}
	return ""
}

func (x *NodeDetailPod) GetCpuLimit() string {
	if x != nil {
		return x.CpuLimit
	}
	return ""
}

func (x *NodeDetailPod) GetMemoryRequest() string {
	if x != nil {
		return x.MemoryRequest
	}
	return ""
}

func (x *NodeDetailPod) GetMemoryLimit() string {
	if x != nil {
		return x.MemoryLimit
	}
	return ""
}

// Response message for GetNodeDetail
type GetNodeDetailResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Node          *NodeDetailInfo        `protobuf:"bytes,1,opt,name=node,proto3" json:"node,omitempty"`
	Pods          []*NodeDetailPod       `protobuf:"bytes,2,rep,name=pods,proto3" json:"pods,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetNodeDetailResponse) Reset() {
	*x = GetNodeDetailResponse{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[29]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetNodeDetailResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetNodeDetailResponse) ProtoMessage() {}

func (x *GetNodeDetailResponse) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[29]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetNodeDetailResponse.ProtoReflect.Descriptor instead.
func (*GetNodeDetailResponse) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{29}
}

func (x *GetNodeDetailResponse) GetNode() *NodeDetailInfo {
	if x != nil {
		return x.Node
	}
	return nil
}

func (x *GetNodeDetailResponse) GetPods() []*NodeDetailPod {
	if x != nil {
		return x.Pods
	}
	return nil
}

// Request message for GetResourceGroupServiceDetail
type GetResourceGroupServiceDetailRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The service kind to filter by (e.g. "engine")
	ServiceKind string `protobuf:"bytes,1,opt,name=service_kind,json=serviceKind,proto3" json:"service_kind,omitempty"`
	// The resource group to filter by (e.g. "default")
	ResourceGroup string `protobuf:"bytes,2,opt,name=resource_group,json=resourceGroup,proto3" json:"resource_group,omitempty"`
	// Start time filter - only include pods observed after this time
	StartTime *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=start_time,json=startTime,proto3" json:"start_time,omitempty"`
	// End time filter - only include pods observed before this time
	EndTime       *timestamppb.Timestamp `protobuf:"bytes,4,opt,name=end_time,json=endTime,proto3" json:"end_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetResourceGroupServiceDetailRequest) Reset() {
	*x = GetResourceGroupServiceDetailRequest{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[30]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetResourceGroupServiceDetailRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetResourceGroupServiceDetailRequest) ProtoMessage() {}

func (x *GetResourceGroupServiceDetailRequest) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[30]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetResourceGroupServiceDetailRequest.ProtoReflect.Descriptor instead.
func (*GetResourceGroupServiceDetailRequest) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{30}
}

func (x *GetResourceGroupServiceDetailRequest) GetServiceKind() string {
	if x != nil {
		return x.ServiceKind
	}
	return ""
}

func (x *GetResourceGroupServiceDetailRequest) GetResourceGroup() string {
	if x != nil {
		return x.ResourceGroup
	}
	return ""
}

func (x *GetResourceGroupServiceDetailRequest) GetStartTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StartTime
	}
	return nil
}

func (x *GetResourceGroupServiceDetailRequest) GetEndTime() *timestamppb.Timestamp {
	if x != nil {
		return x.EndTime
	}
	return nil
}

// Information about a pod in a resource group service
type ResourceGroupServicePod struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	PodName       string                 `protobuf:"bytes,1,opt,name=pod_name,json=podName,proto3" json:"pod_name,omitempty"`
	StartTime     *timestamppb.Timestamp `protobuf:"bytes,2,opt,name=start_time,json=startTime,proto3" json:"start_time,omitempty"`
	EndTime       *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=end_time,json=endTime,proto3" json:"end_time,omitempty"`
	WorkloadType  string                 `protobuf:"bytes,4,opt,name=workload_type,json=workloadType,proto3" json:"workload_type,omitempty"`
	NodeName      string                 `protobuf:"bytes,5,opt,name=node_name,json=nodeName,proto3" json:"node_name,omitempty"`
	CpuRequest    string                 `protobuf:"bytes,6,opt,name=cpu_request,json=cpuRequest,proto3" json:"cpu_request,omitempty"`
	CpuLimit      string                 `protobuf:"bytes,7,opt,name=cpu_limit,json=cpuLimit,proto3" json:"cpu_limit,omitempty"`
	MemoryRequest string                 `protobuf:"bytes,8,opt,name=memory_request,json=memoryRequest,proto3" json:"memory_request,omitempty"`
	MemoryLimit   string                 `protobuf:"bytes,9,opt,name=memory_limit,json=memoryLimit,proto3" json:"memory_limit,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ResourceGroupServicePod) Reset() {
	*x = ResourceGroupServicePod{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[31]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ResourceGroupServicePod) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ResourceGroupServicePod) ProtoMessage() {}

func (x *ResourceGroupServicePod) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[31]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ResourceGroupServicePod.ProtoReflect.Descriptor instead.
func (*ResourceGroupServicePod) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{31}
}

func (x *ResourceGroupServicePod) GetPodName() string {
	if x != nil {
		return x.PodName
	}
	return ""
}

func (x *ResourceGroupServicePod) GetStartTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StartTime
	}
	return nil
}

func (x *ResourceGroupServicePod) GetEndTime() *timestamppb.Timestamp {
	if x != nil {
		return x.EndTime
	}
	return nil
}

func (x *ResourceGroupServicePod) GetWorkloadType() string {
	if x != nil {
		return x.WorkloadType
	}
	return ""
}

func (x *ResourceGroupServicePod) GetNodeName() string {
	if x != nil {
		return x.NodeName
	}
	return ""
}

func (x *ResourceGroupServicePod) GetCpuRequest() string {
	if x != nil {
		return x.CpuRequest
	}
	return ""
}

func (x *ResourceGroupServicePod) GetCpuLimit() string {
	if x != nil {
		return x.CpuLimit
	}
	return ""
}

func (x *ResourceGroupServicePod) GetMemoryRequest() string {
	if x != nil {
		return x.MemoryRequest
	}
	return ""
}

func (x *ResourceGroupServicePod) GetMemoryLimit() string {
	if x != nil {
		return x.MemoryLimit
	}
	return ""
}

// Response message for GetResourceGroupServiceDetail
type GetResourceGroupServiceDetailResponse struct {
	state         protoimpl.MessageState     `protogen:"open.v1"`
	ServiceKind   string                     `protobuf:"bytes,1,opt,name=service_kind,json=serviceKind,proto3" json:"service_kind,omitempty"`
	ResourceGroup string                     `protobuf:"bytes,2,opt,name=resource_group,json=resourceGroup,proto3" json:"resource_group,omitempty"`
	Pods          []*ResourceGroupServicePod `protobuf:"bytes,3,rep,name=pods,proto3" json:"pods,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetResourceGroupServiceDetailResponse) Reset() {
	*x = GetResourceGroupServiceDetailResponse{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[32]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetResourceGroupServiceDetailResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetResourceGroupServiceDetailResponse) ProtoMessage() {}

func (x *GetResourceGroupServiceDetailResponse) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[32]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetResourceGroupServiceDetailResponse.ProtoReflect.Descriptor instead.
func (*GetResourceGroupServiceDetailResponse) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{32}
}

func (x *GetResourceGroupServiceDetailResponse) GetServiceKind() string {
	if x != nil {
		return x.ServiceKind
	}
	return ""
}

func (x *GetResourceGroupServiceDetailResponse) GetResourceGroup() string {
	if x != nil {
		return x.ResourceGroup
	}
	return ""
}

func (x *GetResourceGroupServiceDetailResponse) GetPods() []*ResourceGroupServicePod {
	if x != nil {
		return x.Pods
	}
	return nil
}

type CheckSelfHostedLicenseRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CheckSelfHostedLicenseRequest) Reset() {
	*x = CheckSelfHostedLicenseRequest{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[33]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CheckSelfHostedLicenseRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CheckSelfHostedLicenseRequest) ProtoMessage() {}

func (x *CheckSelfHostedLicenseRequest) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[33]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CheckSelfHostedLicenseRequest.ProtoReflect.Descriptor instead.
func (*CheckSelfHostedLicenseRequest) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{33}
}

// CheckSelfHostedLicenseResponse echoes back the license key the caller
// authenticated with. It is deliberately cheap — no usage data is read — so a
// federating server can poll it as a healthcheck. A successful response is
// itself the proof of validity: the auth layer rejects unknown, revoked and
// expired keys before the handler runs.
type CheckSelfHostedLicenseResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	LicenseKeyId  string                 `protobuf:"bytes,1,opt,name=license_key_id,json=licenseKeyId,proto3" json:"license_key_id,omitempty"`
	TeamId        string                 `protobuf:"bytes,2,opt,name=team_id,json=teamId,proto3" json:"team_id,omitempty"`
	Name          string                 `protobuf:"bytes,3,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CheckSelfHostedLicenseResponse) Reset() {
	*x = CheckSelfHostedLicenseResponse{}
	mi := &file_chalk_server_v1_billing_proto_msgTypes[34]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CheckSelfHostedLicenseResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CheckSelfHostedLicenseResponse) ProtoMessage() {}

func (x *CheckSelfHostedLicenseResponse) ProtoReflect() protoreflect.Message {
	mi := &file_chalk_server_v1_billing_proto_msgTypes[34]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CheckSelfHostedLicenseResponse.ProtoReflect.Descriptor instead.
func (*CheckSelfHostedLicenseResponse) Descriptor() ([]byte, []int) {
	return file_chalk_server_v1_billing_proto_rawDescGZIP(), []int{34}
}

func (x *CheckSelfHostedLicenseResponse) GetLicenseKeyId() string {
	if x != nil {
		return x.LicenseKeyId
	}
	return ""
}

func (x *CheckSelfHostedLicenseResponse) GetTeamId() string {
	if x != nil {
		return x.TeamId
	}
	return ""
}

func (x *CheckSelfHostedLicenseResponse) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

var File_chalk_server_v1_billing_proto protoreflect.FileDescriptor

const file_chalk_server_v1_billing_proto_rawDesc = "" +
	"\n" +
	"\x1dchalk/server/v1/billing.proto\x12\x0fchalk.server.v1\x1a\x1fchalk/auth/v1/permissions.proto\x1a\x1fchalk/kubernetes/v1/nodes.proto\x1a\x1echalk/kubernetes/v1/pods.proto\x1a!chalk/pubsub/v1/node_status.proto\x1a chalk/pubsub/v1/pod_status.proto\x1a\x1bchalk/server/v1/chart.proto\x1a!chalk/server/v1/pod_request.proto\x1a\x19chalk/usage/v1/rate.proto\x1a\x1fgoogle/protobuf/timestamp.proto\x1a\x16google/type/date.proto\x1a\x19google/type/decimal.proto\"\xe1\x02\n" +
	"\x14GetUsageChartRequest\x12\x1e\n" +
	"\bstart_ms\x18\x01 \x01(\x03H\x00R\astartMs\x88\x01\x01\x12\x1a\n" +
	"\x06end_ms\x18\x02 \x01(\x03H\x01R\x05endMs\x88\x01\x01\x12>\n" +
	"\x06period\x18\x03 \x01(\x0e2!.chalk.server.v1.UsageChartPeriodH\x02R\x06period\x88\x01\x01\x12D\n" +
	"\bgrouping\x18\x04 \x01(\x0e2#.chalk.server.v1.UsageChartGroupingH\x03R\bgrouping\x88\x01\x01\x12H\n" +
	"\n" +
	"time_range\x18\x05 \x01(\x0e2$.chalk.server.v1.UsageChartTimeRangeH\x04R\ttimeRange\x88\x01\x01B\v\n" +
	"\t_start_msB\t\n" +
	"\a_end_msB\t\n" +
	"\a_periodB\v\n" +
	"\t_groupingB\r\n" +
	"\v_time_range\"E\n" +
	"\x15GetUsageChartResponse\x12,\n" +
	"\x05chart\x18\x01 \x01(\v2\x16.chalk.server.v1.ChartR\x05chart\"\x1c\n" +
	"\x1aGetUtilizationRatesRequest\"\x89\x02\n" +
	"\x1bGetUtilizationRatesResponse\x121\n" +
	"\x05rates\x18\x01 \x03(\v2\x1b.chalk.usage.v1.MachineRateR\x05rates\x12V\n" +
	"\x1dsandbox_credits_per_vcpu_hour\x18\x02 \x01(\v2\x14.google.type.DecimalR\x19sandboxCreditsPerVcpuHour\x12_\n" +
	"\"sandbox_credits_per_gb_memory_hour\x18\x03 \x01(\v2\x14.google.type.DecimalR\x1dsandboxCreditsPerGbMemoryHour\"\"\n" +
	" GetAvailableInstanceTypesRequest\"m\n" +
	"!GetAvailableInstanceTypesResponse\x12H\n" +
	"\x0einstance_types\x18\x01 \x03(\v2!.chalk.usage.v1.CloudInstanceTypeR\rinstanceTypes\"\xd2\x01\n" +
	"\x16GetNodesAndPodsRequest\x12!\n" +
	"\tnamespace\x18\x01 \x01(\tH\x00R\tnamespace\x88\x01\x01\x121\n" +
	"\x12pod_label_selector\x18\x02 \x01(\tH\x01R\x10podLabelSelector\x88\x01\x01\x12*\n" +
	"\x0eenvironment_id\x18\x03 \x01(\tH\x02R\renvironmentId\x88\x01\x01B\f\n" +
	"\n" +
	"_namespaceB\x15\n" +
	"\x13_pod_label_selectorB\x11\n" +
	"\x0f_environment_id\"\x88\x01\n" +
	"\x17GetNodesAndPodsResponse\x127\n" +
	"\x05nodes\x18\x01 \x03(\v2!.chalk.pubsub.v1.NodeStatusPubSubR\x05nodes\x124\n" +
	"\x04pods\x18\x02 \x03(\v2 .chalk.pubsub.v1.PodStatusPubSubR\x04pods\"R\n" +
	"\x17PublishNodeUsageRequest\x127\n" +
	"\x05nodes\x18\x01 \x03(\v2!.chalk.pubsub.v1.NodeStatusPubSubR\x05nodes\"\x1a\n" +
	"\x18PublishNodeUsageResponse\"\xd4\x01\n" +
	"\x18GetNodesAndPodsUIRequest\x12!\n" +
	"\tnamespace\x18\x01 \x01(\tH\x00R\tnamespace\x88\x01\x01\x121\n" +
	"\x12pod_label_selector\x18\x02 \x01(\tH\x01R\x10podLabelSelector\x88\x01\x01\x12*\n" +
	"\x0eenvironment_id\x18\x03 \x01(\tH\x02R\renvironmentId\x88\x01\x01B\f\n" +
	"\n" +
	"_namespaceB\x15\n" +
	"\x13_pod_label_selectorB\x11\n" +
	"\x0f_environment_id\"\x96\x01\n" +
	"\x19GetNodesAndPodsUIResponse\x12=\n" +
	"\x05nodes\x18\x01 \x03(\v2'.chalk.kubernetes.v1.KubernetesNodeDataR\x05nodes\x12:\n" +
	"\x04pods\x18\x02 \x03(\v2&.chalk.kubernetes.v1.KubernetesPodDataR\x04pods\"\x18\n" +
	"\x16SyncUtilizationRequest\"\x19\n" +
	"\x17SyncUtilizationResponse\"\x19\n" +
	"\x17GetCreditBundlesRequest\"S\n" +
	"\x18GetCreditBundlesResponse\x127\n" +
	"\abundles\x18\x01 \x03(\v2\x1d.chalk.server.v1.CreditBundleR\abundles\"\xa6\x02\n" +
	"\fCreditBundle\x12\x1b\n" +
	"\tbundle_id\x18\x01 \x01(\tR\bbundleId\x126\n" +
	"\rpurchase_date\x18\x02 \x01(\v2\x11.google.type.DateR\fpurchaseDate\x12'\n" +
	"\x0fcredit_quantity\x18\x03 \x01(\x05R\x0ecreditQuantity\x12%\n" +
	"\x0epurchase_price\x18\x04 \x01(\x05R\rpurchasePrice\x125\n" +
	"\n" +
	"expires_on\x18\x05 \x01(\v2\x11.google.type.DateH\x00R\texpiresOn\x88\x01\x01\x12+\n" +
	"\x11remaining_credits\x18\x06 \x01(\x05R\x10remainingCreditsB\r\n" +
	"\v_expires_on\"\xac\x01\n" +
	"\x17GetInstanceUsageRequest\x12\x1e\n" +
	"\bstart_ms\x18\x01 \x01(\x03H\x00R\astartMs\x88\x01\x01\x12\x1a\n" +
	"\x06end_ms\x18\x02 \x01(\x03H\x01R\x05endMs\x88\x01\x01\x12*\n" +
	"\x0eenvironment_id\x18\x03 \x01(\tH\x02R\renvironmentId\x88\x01\x01B\v\n" +
	"\t_start_msB\t\n" +
	"\a_end_msB\x11\n" +
	"\x0f_environment_id\"\xf8\x01\n" +
	"\rInstanceUsage\x12!\n" +
	"\fcluster_name\x18\x01 \x01(\tR\vclusterName\x12#\n" +
	"\rinstance_type\x18\x02 \x01(\tR\finstanceType\x12\x0e\n" +
	"\x02id\x18\x03 \x01(\tR\x02id\x129\n" +
	"\n" +
	"start_time\x18\x04 \x01(\v2\x1a.google.protobuf.TimestampR\tstartTime\x125\n" +
	"\bend_time\x18\x05 \x01(\v2\x1a.google.protobuf.TimestampR\aendTime\x12\x1d\n" +
	"\n" +
	"hours_used\x18\x06 \x01(\x01R\thoursUsed\"X\n" +
	"\x18GetInstanceUsageResponse\x12<\n" +
	"\tinstances\x18\x01 \x03(\v2\x1e.chalk.server.v1.InstanceUsageR\tinstances\"\xe3\x03\n" +
	"\x17GetPodTimeRangesRequest\x12\x1b\n" +
	"\tpod_names\x18\x01 \x03(\tR\bpodNames\x12)\n" +
	"\x0epod_name_regex\x18\x02 \x01(\tH\x00R\fpodNameRegex\x88\x01\x01\x12*\n" +
	"\x0eresource_group\x18\x03 \x01(\tH\x01R\rresourceGroup\x88\x01\x01\x12\x1f\n" +
	"\bnodepool\x18\x04 \x01(\tH\x02R\bnodepool\x88\x01\x01\x12!\n" +
	"\tcomponent\x18\x05 \x01(\tH\x03R\tcomponent\x88\x01\x01\x12&\n" +
	"\fservice_kind\x18\x06 \x01(\tH\x04R\vserviceKind\x88\x01\x01\x12>\n" +
	"\n" +
	"start_time\x18\a \x01(\v2\x1a.google.protobuf.TimestampH\x05R\tstartTime\x88\x01\x01\x12:\n" +
	"\bend_time\x18\b \x01(\v2\x1a.google.protobuf.TimestampH\x06R\aendTime\x88\x01\x01B\x11\n" +
	"\x0f_pod_name_regexB\x11\n" +
	"\x0f_resource_groupB\v\n" +
	"\t_nodepoolB\f\n" +
	"\n" +
	"_componentB\x0f\n" +
	"\r_service_kindB\r\n" +
	"\v_start_timeB\v\n" +
	"\t_end_time\"\xc3\x03\n" +
	"\fPodTimeRange\x12\x19\n" +
	"\bpod_name\x18\x01 \x01(\tR\apodName\x129\n" +
	"\n" +
	"start_time\x18\x02 \x01(\v2\x1a.google.protobuf.TimestampR\tstartTime\x125\n" +
	"\bend_time\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\aendTime\x12\x1b\n" +
	"\tnode_name\x18\x04 \x01(\tR\bnodeName\x12#\n" +
	"\rinstance_type\x18\x05 \x01(\tR\finstanceType\x120\n" +
	"\x14memory_request_bytes\x18\x06 \x01(\x03R\x12memoryRequestBytes\x124\n" +
	"\x16cpu_request_millicores\x18\a \x01(\x03R\x14cpuRequestMillicores\x12A\n" +
	"\x06labels\x18\b \x03(\v2).chalk.server.v1.PodTimeRange.LabelsEntryR\x06labels\x1a9\n" +
	"\vLabelsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"Z\n" +
	"\x18GetPodTimeRangesResponse\x12>\n" +
	"\vtime_ranges\x18\x01 \x03(\v2\x1d.chalk.server.v1.PodTimeRangeR\n" +
	"timeRanges\"\xc0\x02\n" +
	"\x18GetNodeTimeRangesRequest\x12\x1d\n" +
	"\n" +
	"node_names\x18\x01 \x03(\tR\tnodeNames\x12+\n" +
	"\x0fnode_name_regex\x18\x02 \x01(\tH\x00R\rnodeNameRegex\x88\x01\x01\x12\x1f\n" +
	"\bnodepool\x18\x03 \x01(\tH\x01R\bnodepool\x88\x01\x01\x12>\n" +
	"\n" +
	"start_time\x18\x04 \x01(\v2\x1a.google.protobuf.TimestampH\x02R\tstartTime\x88\x01\x01\x12:\n" +
	"\bend_time\x18\x05 \x01(\v2\x1a.google.protobuf.TimestampH\x03R\aendTime\x88\x01\x01B\x12\n" +
	"\x10_node_name_regexB\v\n" +
	"\t_nodepoolB\r\n" +
	"\v_start_timeB\v\n" +
	"\t_end_time\"\xb9\x01\n" +
	"\rNodeTimeRange\x12\x1b\n" +
	"\tnode_name\x18\x01 \x01(\tR\bnodeName\x129\n" +
	"\n" +
	"start_time\x18\x02 \x01(\v2\x1a.google.protobuf.TimestampR\tstartTime\x125\n" +
	"\bend_time\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\aendTime\x12\x19\n" +
	"\bnode_uid\x18\x04 \x01(\tR\anodeUid\"\\\n" +
	"\x19GetNodeTimeRangesResponse\x12?\n" +
	"\vtime_ranges\x18\x01 \x03(\v2\x1e.chalk.server.v1.NodeTimeRangeR\n" +
	"timeRanges\"\x83\x01\n" +
	"\x14GetNodeDetailRequest\x12!\n" +
	"\fcluster_name\x18\x01 \x01(\tR\vclusterName\x12\x1b\n" +
	"\tnode_name\x18\x02 \x01(\tR\bnodeName\x12\x1e\n" +
	"\bnode_uid\x18\x03 \x01(\tH\x00R\anodeUid\x88\x01\x01B\v\n" +
	"\t_node_uid\"\x9e\x02\n" +
	"\x0eNodeDetailInfo\x12\x1b\n" +
	"\tnode_name\x18\x01 \x01(\tR\bnodeName\x12!\n" +
	"\fcluster_name\x18\x02 \x01(\tR\vclusterName\x12\x1a\n" +
	"\bnodepool\x18\x03 \x01(\tR\bnodepool\x12#\n" +
	"\rinstance_type\x18\x04 \x01(\tR\finstanceType\x129\n" +
	"\n" +
	"start_time\x18\x05 \x01(\v2\x1a.google.protobuf.TimestampR\tstartTime\x125\n" +
	"\bend_time\x18\x06 \x01(\v2\x1a.google.protobuf.TimestampR\aendTime\x12\x19\n" +
	"\bnode_uid\x18\a \x01(\tR\anodeUid\"\xf0\x02\n" +
	"\rNodeDetailPod\x12\x19\n" +
	"\bpod_name\x18\x01 \x01(\tR\apodName\x129\n" +
	"\n" +
	"start_time\x18\x02 \x01(\v2\x1a.google.protobuf.TimestampR\tstartTime\x125\n" +
	"\bend_time\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\aendTime\x12#\n" +
	"\rworkload_type\x18\x04 \x01(\tR\fworkloadType\x12%\n" +
	"\x0eresource_group\x18\x05 \x01(\tR\rresourceGroup\x12\x1f\n" +
	"\vcpu_request\x18\x06 \x01(\tR\n" +
	"cpuRequest\x12\x1b\n" +
	"\tcpu_limit\x18\a \x01(\tR\bcpuLimit\x12%\n" +
	"\x0ememory_request\x18\b \x01(\tR\rmemoryRequest\x12!\n" +
	"\fmemory_limit\x18\t \x01(\tR\vmemoryLimit\"\x80\x01\n" +
	"\x15GetNodeDetailResponse\x123\n" +
	"\x04node\x18\x01 \x01(\v2\x1f.chalk.server.v1.NodeDetailInfoR\x04node\x122\n" +
	"\x04pods\x18\x02 \x03(\v2\x1e.chalk.server.v1.NodeDetailPodR\x04pods\"\xe2\x01\n" +
	"$GetResourceGroupServiceDetailRequest\x12!\n" +
	"\fservice_kind\x18\x01 \x01(\tR\vserviceKind\x12%\n" +
	"\x0eresource_group\x18\x02 \x01(\tR\rresourceGroup\x129\n" +
	"\n" +
	"start_time\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\tstartTime\x125\n" +
	"\bend_time\x18\x04 \x01(\v2\x1a.google.protobuf.TimestampR\aendTime\"\xf0\x02\n" +
	"\x17ResourceGroupServicePod\x12\x19\n" +
	"\bpod_name\x18\x01 \x01(\tR\apodName\x129\n" +
	"\n" +
	"start_time\x18\x02 \x01(\v2\x1a.google.protobuf.TimestampR\tstartTime\x125\n" +
	"\bend_time\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\aendTime\x12#\n" +
	"\rworkload_type\x18\x04 \x01(\tR\fworkloadType\x12\x1b\n" +
	"\tnode_name\x18\x05 \x01(\tR\bnodeName\x12\x1f\n" +
	"\vcpu_request\x18\x06 \x01(\tR\n" +
	"cpuRequest\x12\x1b\n" +
	"\tcpu_limit\x18\a \x01(\tR\bcpuLimit\x12%\n" +
	"\x0ememory_request\x18\b \x01(\tR\rmemoryRequest\x12!\n" +
	"\fmemory_limit\x18\t \x01(\tR\vmemoryLimit\"\xaf\x01\n" +
	"%GetResourceGroupServiceDetailResponse\x12!\n" +
	"\fservice_kind\x18\x01 \x01(\tR\vserviceKind\x12%\n" +
	"\x0eresource_group\x18\x02 \x01(\tR\rresourceGroup\x12<\n" +
	"\x04pods\x18\x03 \x03(\v2(.chalk.server.v1.ResourceGroupServicePodR\x04pods\"\x1f\n" +
	"\x1dCheckSelfHostedLicenseRequest\"s\n" +
	"\x1eCheckSelfHostedLicenseResponse\x12$\n" +
	"\x0elicense_key_id\x18\x01 \x01(\tR\flicenseKeyId\x12\x17\n" +
	"\ateam_id\x18\x02 \x01(\tR\x06teamId\x12\x12\n" +
	"\x04name\x18\x03 \x01(\tR\x04name*\x93\x01\n" +
	"\x10UsageChartPeriod\x12\"\n" +
	"\x1eUSAGE_CHART_PERIOD_UNSPECIFIED\x10\x00\x12\x1c\n" +
	"\x18USAGE_CHART_PERIOD_DAILY\x10\x01\x12\x1e\n" +
	"\x1aUSAGE_CHART_PERIOD_MONTHLY\x10\x02\x12\x1d\n" +
	"\x19USAGE_CHART_PERIOD_HOURLY\x10\x03*\xcf\x01\n" +
	"\x12UsageChartGrouping\x12$\n" +
	" USAGE_CHART_GROUPING_UNSPECIFIED\x10\x00\x12&\n" +
	"\"USAGE_CHART_GROUPING_INSTANCE_TYPE\x10\x01\x12 \n" +
	"\x1cUSAGE_CHART_GROUPING_CLUSTER\x10\x02\x12!\n" +
	"\x1dUSAGE_CHART_GROUPING_NODEPOOL\x10\x03\x12&\n" +
	"\"USAGE_CHART_GROUPING_WORKLOAD_TYPE\x10\x04*{\n" +
	"\x13UsageChartTimeRange\x12&\n" +
	"\"USAGE_CHART_TIME_RANGE_UNSPECIFIED\x10\x00\x12\x1d\n" +
	"\x19USAGE_CHART_TIME_RANGE_1D\x10\x01\x12\x1d\n" +
	"\x19USAGE_CHART_TIME_RANGE_7D\x10\x022\x83\x0e\n" +
	"\x0eBillingService\x12r\n" +
	"\x11GetNodesAndPodsUI\x12).chalk.server.v1.GetNodesAndPodsUIRequest\x1a*.chalk.server.v1.GetNodesAndPodsUIResponse\"\x06\x80}\v\x90\x02\x01\x12l\n" +
	"\x0fGetNodesAndPods\x12'.chalk.server.v1.GetNodesAndPodsRequest\x1a(.chalk.server.v1.GetNodesAndPodsResponse\"\x06\x80}\v\x90\x02\x01\x12l\n" +
	"\x10PublishNodeUsage\x12(.chalk.server.v1.PublishNodeUsageRequest\x1a).chalk.server.v1.PublishNodeUsageResponse\"\x03\x80}\x01\x12f\n" +
	"\rGetUsageChart\x12%.chalk.server.v1.GetUsageChartRequest\x1a&.chalk.server.v1.GetUsageChartResponse\"\x06\x80}\x1c\x90\x02\x01\x12x\n" +
	"\x13GetUtilizationRates\x12+.chalk.server.v1.GetUtilizationRatesRequest\x1a,.chalk.server.v1.GetUtilizationRatesResponse\"\x06\x80}\x01\x90\x02\x01\x12\x8a\x01\n" +
	"\x19GetAvailableInstanceTypes\x121.chalk.server.v1.GetAvailableInstanceTypesRequest\x1a2.chalk.server.v1.GetAvailableInstanceTypesResponse\"\x06\x80}\x01\x90\x02\x01\x12x\n" +
	"\x13GetPodRequestCharts\x12+.chalk.server.v1.GetPodRequestChartsRequest\x1a,.chalk.server.v1.GetPodRequestChartsResponse\"\x06\x80}\x06\x90\x02\x01\x12l\n" +
	"\x0fSyncUtilization\x12'.chalk.server.v1.SyncUtilizationRequest\x1a(.chalk.server.v1.SyncUtilizationResponse\"\x06\x80}\x1b\x90\x02\x01\x12o\n" +
	"\x10GetCreditBundles\x12(.chalk.server.v1.GetCreditBundlesRequest\x1a).chalk.server.v1.GetCreditBundlesResponse\"\x06\x80}\x1c\x90\x02\x01\x12o\n" +
	"\x10GetInstanceUsage\x12(.chalk.server.v1.GetInstanceUsageRequest\x1a).chalk.server.v1.GetInstanceUsageResponse\"\x06\x80}\x1c\x90\x02\x01\x12o\n" +
	"\x10GetPodTimeRanges\x12(.chalk.server.v1.GetPodTimeRangesRequest\x1a).chalk.server.v1.GetPodTimeRangesResponse\"\x06\x80}\x1c\x90\x02\x01\x12r\n" +
	"\x11GetNodeTimeRanges\x12).chalk.server.v1.GetNodeTimeRangesRequest\x1a*.chalk.server.v1.GetNodeTimeRangesResponse\"\x06\x80}\x1c\x90\x02\x01\x12f\n" +
	"\rGetNodeDetail\x12%.chalk.server.v1.GetNodeDetailRequest\x1a&.chalk.server.v1.GetNodeDetailResponse\"\x06\x80}\x1c\x90\x02\x01\x12\x96\x01\n" +
	"\x1dGetResourceGroupServiceDetail\x125.chalk.server.v1.GetResourceGroupServiceDetailRequest\x1a6.chalk.server.v1.GetResourceGroupServiceDetailResponse\"\x06\x80}\x1c\x90\x02\x01\x12\x81\x01\n" +
	"\x16CheckSelfHostedLicense\x12..chalk.server.v1.CheckSelfHostedLicenseRequest\x1a/.chalk.server.v1.CheckSelfHostedLicenseResponse\"\x06\x80}\x1c\x90\x02\x01B\xbc\x01\n" +
	"\x13com.chalk.server.v1B\fBillingProtoP\x01Z9github.com/chalk-ai/chalk-go/gen/chalk/server/v1;serverv1\xa2\x02\x03CSX\xaa\x02\x0fChalk.Server.V1\xca\x02\x0fChalk\\Server\\V1\xe2\x02\x1bChalk\\Server\\V1\\GPBMetadata\xea\x02\x11Chalk::Server::V1b\x06proto3"

var (
	file_chalk_server_v1_billing_proto_rawDescOnce sync.Once
	file_chalk_server_v1_billing_proto_rawDescData []byte
)

func file_chalk_server_v1_billing_proto_rawDescGZIP() []byte {
	file_chalk_server_v1_billing_proto_rawDescOnce.Do(func() {
		file_chalk_server_v1_billing_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_chalk_server_v1_billing_proto_rawDesc), len(file_chalk_server_v1_billing_proto_rawDesc)))
	})
	return file_chalk_server_v1_billing_proto_rawDescData
}

var file_chalk_server_v1_billing_proto_enumTypes = make([]protoimpl.EnumInfo, 3)
var file_chalk_server_v1_billing_proto_msgTypes = make([]protoimpl.MessageInfo, 36)
var file_chalk_server_v1_billing_proto_goTypes = []any{
	(UsageChartPeriod)(0),                         // 0: chalk.server.v1.UsageChartPeriod
	(UsageChartGrouping)(0),                       // 1: chalk.server.v1.UsageChartGrouping
	(UsageChartTimeRange)(0),                      // 2: chalk.server.v1.UsageChartTimeRange
	(*GetUsageChartRequest)(nil),                  // 3: chalk.server.v1.GetUsageChartRequest
	(*GetUsageChartResponse)(nil),                 // 4: chalk.server.v1.GetUsageChartResponse
	(*GetUtilizationRatesRequest)(nil),            // 5: chalk.server.v1.GetUtilizationRatesRequest
	(*GetUtilizationRatesResponse)(nil),           // 6: chalk.server.v1.GetUtilizationRatesResponse
	(*GetAvailableInstanceTypesRequest)(nil),      // 7: chalk.server.v1.GetAvailableInstanceTypesRequest
	(*GetAvailableInstanceTypesResponse)(nil),     // 8: chalk.server.v1.GetAvailableInstanceTypesResponse
	(*GetNodesAndPodsRequest)(nil),                // 9: chalk.server.v1.GetNodesAndPodsRequest
	(*GetNodesAndPodsResponse)(nil),               // 10: chalk.server.v1.GetNodesAndPodsResponse
	(*PublishNodeUsageRequest)(nil),               // 11: chalk.server.v1.PublishNodeUsageRequest
	(*PublishNodeUsageResponse)(nil),              // 12: chalk.server.v1.PublishNodeUsageResponse
	(*GetNodesAndPodsUIRequest)(nil),              // 13: chalk.server.v1.GetNodesAndPodsUIRequest
	(*GetNodesAndPodsUIResponse)(nil),             // 14: chalk.server.v1.GetNodesAndPodsUIResponse
	(*SyncUtilizationRequest)(nil),                // 15: chalk.server.v1.SyncUtilizationRequest
	(*SyncUtilizationResponse)(nil),               // 16: chalk.server.v1.SyncUtilizationResponse
	(*GetCreditBundlesRequest)(nil),               // 17: chalk.server.v1.GetCreditBundlesRequest
	(*GetCreditBundlesResponse)(nil),              // 18: chalk.server.v1.GetCreditBundlesResponse
	(*CreditBundle)(nil),                          // 19: chalk.server.v1.CreditBundle
	(*GetInstanceUsageRequest)(nil),               // 20: chalk.server.v1.GetInstanceUsageRequest
	(*InstanceUsage)(nil),                         // 21: chalk.server.v1.InstanceUsage
	(*GetInstanceUsageResponse)(nil),              // 22: chalk.server.v1.GetInstanceUsageResponse
	(*GetPodTimeRangesRequest)(nil),               // 23: chalk.server.v1.GetPodTimeRangesRequest
	(*PodTimeRange)(nil),                          // 24: chalk.server.v1.PodTimeRange
	(*GetPodTimeRangesResponse)(nil),              // 25: chalk.server.v1.GetPodTimeRangesResponse
	(*GetNodeTimeRangesRequest)(nil),              // 26: chalk.server.v1.GetNodeTimeRangesRequest
	(*NodeTimeRange)(nil),                         // 27: chalk.server.v1.NodeTimeRange
	(*GetNodeTimeRangesResponse)(nil),             // 28: chalk.server.v1.GetNodeTimeRangesResponse
	(*GetNodeDetailRequest)(nil),                  // 29: chalk.server.v1.GetNodeDetailRequest
	(*NodeDetailInfo)(nil),                        // 30: chalk.server.v1.NodeDetailInfo
	(*NodeDetailPod)(nil),                         // 31: chalk.server.v1.NodeDetailPod
	(*GetNodeDetailResponse)(nil),                 // 32: chalk.server.v1.GetNodeDetailResponse
	(*GetResourceGroupServiceDetailRequest)(nil),  // 33: chalk.server.v1.GetResourceGroupServiceDetailRequest
	(*ResourceGroupServicePod)(nil),               // 34: chalk.server.v1.ResourceGroupServicePod
	(*GetResourceGroupServiceDetailResponse)(nil), // 35: chalk.server.v1.GetResourceGroupServiceDetailResponse
	(*CheckSelfHostedLicenseRequest)(nil),         // 36: chalk.server.v1.CheckSelfHostedLicenseRequest
	(*CheckSelfHostedLicenseResponse)(nil),        // 37: chalk.server.v1.CheckSelfHostedLicenseResponse
	nil,                                           // 38: chalk.server.v1.PodTimeRange.LabelsEntry
	(*Chart)(nil),                                 // 39: chalk.server.v1.Chart
	(*v1.MachineRate)(nil),                        // 40: chalk.usage.v1.MachineRate
	(*decimal.Decimal)(nil),                       // 41: google.type.Decimal
	(*v1.CloudInstanceType)(nil),                  // 42: chalk.usage.v1.CloudInstanceType
	(*v11.NodeStatusPubSub)(nil),                  // 43: chalk.pubsub.v1.NodeStatusPubSub
	(*v11.PodStatusPubSub)(nil),                   // 44: chalk.pubsub.v1.PodStatusPubSub
	(*v12.KubernetesNodeData)(nil),                // 45: chalk.kubernetes.v1.KubernetesNodeData
	(*v12.KubernetesPodData)(nil),                 // 46: chalk.kubernetes.v1.KubernetesPodData
	(*date.Date)(nil),                             // 47: google.type.Date
	(*timestamppb.Timestamp)(nil),                 // 48: google.protobuf.Timestamp
	(*GetPodRequestChartsRequest)(nil),            // 49: chalk.server.v1.GetPodRequestChartsRequest
	(*GetPodRequestChartsResponse)(nil),           // 50: chalk.server.v1.GetPodRequestChartsResponse
}
var file_chalk_server_v1_billing_proto_depIdxs = []int32{
	0,  // 0: chalk.server.v1.GetUsageChartRequest.period:type_name -> chalk.server.v1.UsageChartPeriod
	1,  // 1: chalk.server.v1.GetUsageChartRequest.grouping:type_name -> chalk.server.v1.UsageChartGrouping
	2,  // 2: chalk.server.v1.GetUsageChartRequest.time_range:type_name -> chalk.server.v1.UsageChartTimeRange
	39, // 3: chalk.server.v1.GetUsageChartResponse.chart:type_name -> chalk.server.v1.Chart
	40, // 4: chalk.server.v1.GetUtilizationRatesResponse.rates:type_name -> chalk.usage.v1.MachineRate
	41, // 5: chalk.server.v1.GetUtilizationRatesResponse.sandbox_credits_per_vcpu_hour:type_name -> google.type.Decimal
	41, // 6: chalk.server.v1.GetUtilizationRatesResponse.sandbox_credits_per_gb_memory_hour:type_name -> google.type.Decimal
	42, // 7: chalk.server.v1.GetAvailableInstanceTypesResponse.instance_types:type_name -> chalk.usage.v1.CloudInstanceType
	43, // 8: chalk.server.v1.GetNodesAndPodsResponse.nodes:type_name -> chalk.pubsub.v1.NodeStatusPubSub
	44, // 9: chalk.server.v1.GetNodesAndPodsResponse.pods:type_name -> chalk.pubsub.v1.PodStatusPubSub
	43, // 10: chalk.server.v1.PublishNodeUsageRequest.nodes:type_name -> chalk.pubsub.v1.NodeStatusPubSub
	45, // 11: chalk.server.v1.GetNodesAndPodsUIResponse.nodes:type_name -> chalk.kubernetes.v1.KubernetesNodeData
	46, // 12: chalk.server.v1.GetNodesAndPodsUIResponse.pods:type_name -> chalk.kubernetes.v1.KubernetesPodData
	19, // 13: chalk.server.v1.GetCreditBundlesResponse.bundles:type_name -> chalk.server.v1.CreditBundle
	47, // 14: chalk.server.v1.CreditBundle.purchase_date:type_name -> google.type.Date
	47, // 15: chalk.server.v1.CreditBundle.expires_on:type_name -> google.type.Date
	48, // 16: chalk.server.v1.InstanceUsage.start_time:type_name -> google.protobuf.Timestamp
	48, // 17: chalk.server.v1.InstanceUsage.end_time:type_name -> google.protobuf.Timestamp
	21, // 18: chalk.server.v1.GetInstanceUsageResponse.instances:type_name -> chalk.server.v1.InstanceUsage
	48, // 19: chalk.server.v1.GetPodTimeRangesRequest.start_time:type_name -> google.protobuf.Timestamp
	48, // 20: chalk.server.v1.GetPodTimeRangesRequest.end_time:type_name -> google.protobuf.Timestamp
	48, // 21: chalk.server.v1.PodTimeRange.start_time:type_name -> google.protobuf.Timestamp
	48, // 22: chalk.server.v1.PodTimeRange.end_time:type_name -> google.protobuf.Timestamp
	38, // 23: chalk.server.v1.PodTimeRange.labels:type_name -> chalk.server.v1.PodTimeRange.LabelsEntry
	24, // 24: chalk.server.v1.GetPodTimeRangesResponse.time_ranges:type_name -> chalk.server.v1.PodTimeRange
	48, // 25: chalk.server.v1.GetNodeTimeRangesRequest.start_time:type_name -> google.protobuf.Timestamp
	48, // 26: chalk.server.v1.GetNodeTimeRangesRequest.end_time:type_name -> google.protobuf.Timestamp
	48, // 27: chalk.server.v1.NodeTimeRange.start_time:type_name -> google.protobuf.Timestamp
	48, // 28: chalk.server.v1.NodeTimeRange.end_time:type_name -> google.protobuf.Timestamp
	27, // 29: chalk.server.v1.GetNodeTimeRangesResponse.time_ranges:type_name -> chalk.server.v1.NodeTimeRange
	48, // 30: chalk.server.v1.NodeDetailInfo.start_time:type_name -> google.protobuf.Timestamp
	48, // 31: chalk.server.v1.NodeDetailInfo.end_time:type_name -> google.protobuf.Timestamp
	48, // 32: chalk.server.v1.NodeDetailPod.start_time:type_name -> google.protobuf.Timestamp
	48, // 33: chalk.server.v1.NodeDetailPod.end_time:type_name -> google.protobuf.Timestamp
	30, // 34: chalk.server.v1.GetNodeDetailResponse.node:type_name -> chalk.server.v1.NodeDetailInfo
	31, // 35: chalk.server.v1.GetNodeDetailResponse.pods:type_name -> chalk.server.v1.NodeDetailPod
	48, // 36: chalk.server.v1.GetResourceGroupServiceDetailRequest.start_time:type_name -> google.protobuf.Timestamp
	48, // 37: chalk.server.v1.GetResourceGroupServiceDetailRequest.end_time:type_name -> google.protobuf.Timestamp
	48, // 38: chalk.server.v1.ResourceGroupServicePod.start_time:type_name -> google.protobuf.Timestamp
	48, // 39: chalk.server.v1.ResourceGroupServicePod.end_time:type_name -> google.protobuf.Timestamp
	34, // 40: chalk.server.v1.GetResourceGroupServiceDetailResponse.pods:type_name -> chalk.server.v1.ResourceGroupServicePod
	13, // 41: chalk.server.v1.BillingService.GetNodesAndPodsUI:input_type -> chalk.server.v1.GetNodesAndPodsUIRequest
	9,  // 42: chalk.server.v1.BillingService.GetNodesAndPods:input_type -> chalk.server.v1.GetNodesAndPodsRequest
	11, // 43: chalk.server.v1.BillingService.PublishNodeUsage:input_type -> chalk.server.v1.PublishNodeUsageRequest
	3,  // 44: chalk.server.v1.BillingService.GetUsageChart:input_type -> chalk.server.v1.GetUsageChartRequest
	5,  // 45: chalk.server.v1.BillingService.GetUtilizationRates:input_type -> chalk.server.v1.GetUtilizationRatesRequest
	7,  // 46: chalk.server.v1.BillingService.GetAvailableInstanceTypes:input_type -> chalk.server.v1.GetAvailableInstanceTypesRequest
	49, // 47: chalk.server.v1.BillingService.GetPodRequestCharts:input_type -> chalk.server.v1.GetPodRequestChartsRequest
	15, // 48: chalk.server.v1.BillingService.SyncUtilization:input_type -> chalk.server.v1.SyncUtilizationRequest
	17, // 49: chalk.server.v1.BillingService.GetCreditBundles:input_type -> chalk.server.v1.GetCreditBundlesRequest
	20, // 50: chalk.server.v1.BillingService.GetInstanceUsage:input_type -> chalk.server.v1.GetInstanceUsageRequest
	23, // 51: chalk.server.v1.BillingService.GetPodTimeRanges:input_type -> chalk.server.v1.GetPodTimeRangesRequest
	26, // 52: chalk.server.v1.BillingService.GetNodeTimeRanges:input_type -> chalk.server.v1.GetNodeTimeRangesRequest
	29, // 53: chalk.server.v1.BillingService.GetNodeDetail:input_type -> chalk.server.v1.GetNodeDetailRequest
	33, // 54: chalk.server.v1.BillingService.GetResourceGroupServiceDetail:input_type -> chalk.server.v1.GetResourceGroupServiceDetailRequest
	36, // 55: chalk.server.v1.BillingService.CheckSelfHostedLicense:input_type -> chalk.server.v1.CheckSelfHostedLicenseRequest
	14, // 56: chalk.server.v1.BillingService.GetNodesAndPodsUI:output_type -> chalk.server.v1.GetNodesAndPodsUIResponse
	10, // 57: chalk.server.v1.BillingService.GetNodesAndPods:output_type -> chalk.server.v1.GetNodesAndPodsResponse
	12, // 58: chalk.server.v1.BillingService.PublishNodeUsage:output_type -> chalk.server.v1.PublishNodeUsageResponse
	4,  // 59: chalk.server.v1.BillingService.GetUsageChart:output_type -> chalk.server.v1.GetUsageChartResponse
	6,  // 60: chalk.server.v1.BillingService.GetUtilizationRates:output_type -> chalk.server.v1.GetUtilizationRatesResponse
	8,  // 61: chalk.server.v1.BillingService.GetAvailableInstanceTypes:output_type -> chalk.server.v1.GetAvailableInstanceTypesResponse
	50, // 62: chalk.server.v1.BillingService.GetPodRequestCharts:output_type -> chalk.server.v1.GetPodRequestChartsResponse
	16, // 63: chalk.server.v1.BillingService.SyncUtilization:output_type -> chalk.server.v1.SyncUtilizationResponse
	18, // 64: chalk.server.v1.BillingService.GetCreditBundles:output_type -> chalk.server.v1.GetCreditBundlesResponse
	22, // 65: chalk.server.v1.BillingService.GetInstanceUsage:output_type -> chalk.server.v1.GetInstanceUsageResponse
	25, // 66: chalk.server.v1.BillingService.GetPodTimeRanges:output_type -> chalk.server.v1.GetPodTimeRangesResponse
	28, // 67: chalk.server.v1.BillingService.GetNodeTimeRanges:output_type -> chalk.server.v1.GetNodeTimeRangesResponse
	32, // 68: chalk.server.v1.BillingService.GetNodeDetail:output_type -> chalk.server.v1.GetNodeDetailResponse
	35, // 69: chalk.server.v1.BillingService.GetResourceGroupServiceDetail:output_type -> chalk.server.v1.GetResourceGroupServiceDetailResponse
	37, // 70: chalk.server.v1.BillingService.CheckSelfHostedLicense:output_type -> chalk.server.v1.CheckSelfHostedLicenseResponse
	56, // [56:71] is the sub-list for method output_type
	41, // [41:56] is the sub-list for method input_type
	41, // [41:41] is the sub-list for extension type_name
	41, // [41:41] is the sub-list for extension extendee
	0,  // [0:41] is the sub-list for field type_name
}

func init() { file_chalk_server_v1_billing_proto_init() }
func file_chalk_server_v1_billing_proto_init() {
	if File_chalk_server_v1_billing_proto != nil {
		return
	}
	file_chalk_server_v1_chart_proto_init()
	file_chalk_server_v1_pod_request_proto_init()
	file_chalk_server_v1_billing_proto_msgTypes[0].OneofWrappers = []any{}
	file_chalk_server_v1_billing_proto_msgTypes[6].OneofWrappers = []any{}
	file_chalk_server_v1_billing_proto_msgTypes[10].OneofWrappers = []any{}
	file_chalk_server_v1_billing_proto_msgTypes[16].OneofWrappers = []any{}
	file_chalk_server_v1_billing_proto_msgTypes[17].OneofWrappers = []any{}
	file_chalk_server_v1_billing_proto_msgTypes[20].OneofWrappers = []any{}
	file_chalk_server_v1_billing_proto_msgTypes[23].OneofWrappers = []any{}
	file_chalk_server_v1_billing_proto_msgTypes[26].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_chalk_server_v1_billing_proto_rawDesc), len(file_chalk_server_v1_billing_proto_rawDesc)),
			NumEnums:      3,
			NumMessages:   36,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_chalk_server_v1_billing_proto_goTypes,
		DependencyIndexes: file_chalk_server_v1_billing_proto_depIdxs,
		EnumInfos:         file_chalk_server_v1_billing_proto_enumTypes,
		MessageInfos:      file_chalk_server_v1_billing_proto_msgTypes,
	}.Build()
	File_chalk_server_v1_billing_proto = out.File
	file_chalk_server_v1_billing_proto_goTypes = nil
	file_chalk_server_v1_billing_proto_depIdxs = nil
}
