...
func (c *ChalkClientImpl) sendRequest(args sendRequestParams) error {
	...
	if !args.DontRefresh {
		...
		if upsertJwtErr != nil {
			(*c.logger).Errorf("Error pre-emptively refreshing access token: %s", upsertJwtErr.Error())
		}
	}
...
func (c *ChalkClientImpl) retryRequest(originalRequest http.Request, originalBodyBytes []byte, originalResponse *http.Response, originalError error) (*http.Response, error) {
	...
	if upsertJwtUpon401Err != nil {
		(*c.logger).Warnf("Error refreshing access token upon 401: %s", upsertJwtUpon401Err.Error())
...
func getHttpError(logger *LeveledLogger, res http.Response, req http.Request) ChalkHttpError {
	...
	(*logger).Debugf("API error response", err, errorResponse, string(out))
